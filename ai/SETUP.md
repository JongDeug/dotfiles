...
`skills`·`agents`·`commands`가 개별 심링크인 이유는 여러 출처가 한 자리로 모이기 때문이다. `skills`는 추가로, 외부 스킬이 `~/.claude/skills/`에 자기 것을 설치하므로 repo를 통째로 걸면 설치물이 repo로 샌다.
...
- **`ponytail` 플러그인** — `settings.json`의 `enabledPlugins`로 자동 설치되지만 첫 세팅 땐 아직 안 받아졌을 수 있다. 스킬 목록에 ponytail 패밀리가 없으면 `/plugin` → Marketplaces에서 수동 설치. cosmos의 dev·ops가 런타임에 불러 쓴다.
...
- `/cosmos` 커맨드와 `cosmos-*` 서브에이전트가 보이는지
...
```
/cosmos ai/shared/harness/specs/hello-world-api.md
```
