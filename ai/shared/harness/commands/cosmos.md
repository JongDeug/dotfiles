...
- **원본 트리 루트**(방금 만든 워크트리가 아니라, 사용자가 일하던 그 체크아웃)의 `LEARNING.md`가 있으면 읽고 다음 프롬프트에 넘깁니다(planner Constitution과 같은 출처). 미추적이면 워크트리에는 애초에 없습니다. 컨텍스트를 아끼려고 발췌하지 마세요 — 짧은 파일이고, 잘라내면 dev/qe가 그 gotcha를 못 봅니다. 수백 줄로 커졌을 때만 관련 항목을 고릅니다.
- `telemetry.jsonl`은 **원본 트리 루트**에 둡니다 — `LEARNING.md`와 같은 자리입니다. spec이 워크트리 안에 있더라도 telemetry는 원본 트리에 씁니다. 파일이 없어도 됩니다 — 5번의 첫 append가 만듭니다.
- **워크트리 안에 telemetry를 만들지 마세요.** 그 한 줄은 실행 로그가 아니라 **transcript로 가는 색인**입니다. transcript는 `~/.claude/projects/<슬러그>/<세션UUID>/subagents/workflows/wf_<런ID>/`에 남아 워크트리를 지워도 살아남지만, 경로를 잃으면 수백 MB 안에서 그 실행을 다시 찾을 방법이 없습니다. 본문보다 색인이 먼저 죽으면 안 됩니다.
- 워크트리와 함께 사라져도 되는 것은 **구현 결과물뿐**입니다. 원본 트리에 남아야 하는 둘: `LEARNING.md`(교훈)와 `telemetry.jsonl`(색인).
...
**쓰는 곳은 원본 트리입니다 — 워크트리가 아닙니다.** 워크트리는 사용자가 결과를 확인한 뒤 `git worktree remove`로 지웁니다. 거기 쓰면 이번 실행에서 배운 것이 그때 통째로 사라집니다. dev/qe/ops에게는 워크트리 경로를 주지만, 이 한 줄만은 원본 트리 루트에 씁니다.
